...
if env['platform'] == 'linux':
...
elif env['platform'] == 'ios' or env['platform'] == 'iphone':
    bin_suffix = '.a'
    ...
    if host_platform == 'windows' and not env['use_mingw']:
        ...
        if env['target'] == 'debug':
            env.Append(CCFLAGS=['/Z7', '/Od', '/EHsc', '/D_DEBUG', '/MDd', '/ZW'])
        elif env['target'] == 'release':
            env.Append(CCFLAGS=['/O2', '/EHsc', '/DNDEBUG', '/MD', '/ZW'])
...
gamesparks_bindings_path = "gamesparks-cpp/bin/"


# gamesparks
gamesparks_headers_path = "gamesparks-cpp/include/"
gamesparks_src = "gamesparks-cpp/src/"

env.Append(LIBPATH=[godot_bindings_path, gamesparks_bindings_path])
env.Append(CPPPATH=[
    '.',
    'godot-cpp',
...
if env['platform'] == 'ios' or env['platform'] == 'iphone':
    arch_suffix = env['ios_arch']

godot_library = 'libgodot-cpp.{}.{}.{}{}'.format(
    env['platform'],
    env['target'],
    arch_suffix,
    env['LIBSUFFIX'])

gamespark_library = 'libgamesparks-cpp.{}.{}.{}{}'.format(
    env['platform'],
    env['target'],
    arch_suffix,
    env['LIBSUFFIX'])

print('link linrary -> ' + godot_bindings_path + godot_library)
print('link linrary -> ' + gamesparks_bindings_path + gamespark_library)
env.Append(LIBS=[godot_library, gamespark_library])

target_path = 'bin/' + 'gamesparks.{}.{}.{}{}'.format(
        env['platform'],
        env['target'],
        arch_suffix,
        bin_suffix)
